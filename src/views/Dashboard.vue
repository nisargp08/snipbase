                        ...
                        <div class="column is-4-desktop is-6-tablet" v-for="post in posts" :key="post.id">
                            <div class="card">
                                ...
                                <div class="card-content">
                                    ...
                                    <p><a @click="viewPost(post.id)">Read More</a></p>
                                </div>
                                <div class="card-footer">
                                    <span class="card-footer-item">
                                        <a @click="toggleCommentsModal(post)"><i class="fas fa-comments has-text-primary pr-2"></i>Comments ({{ post.comments}})</a>
                                    </span>
                                    <span class="card-footer-item">
                                        <a @click="likePost(post.id,post.likes)"><i class="fas fa-heart has-text-danger pr-2"></i>Likes ({{ post.likes}})</a>
                                    </span>
                                </div>
                            </div>
                        </div>
